...
const emptyForm: SaveShortcutInput = { scope: "global", name: "", commandTemplate: "", description: "", tags: [], enabled: true };
...
export function ShortcutManager({ serverId, open, onClose }: ShortcutManagerProps) {
  ...
  const visible = useMemo(() => {
    ...
    return (shortcuts.data ?? []).filter((item) => !needle || [item.name, item.commandTemplate, item.description, item.tags.join(" ")].join(" ").toLocaleLowerCase().includes(needle));
  }, [query, shortcuts.data]);
  if (!open) return null;
  ...
  const edit = (item: ShortcutRecord) => {
    setEditing(item.id);
    setForm({ id: item.id, scope: item.scope, serverId: item.serverId ?? undefined, name: item.name, commandTemplate: item.commandTemplate, description: item.description, tags: item.tags, enabled: item.enabled });
  };
