...
function OverviewContent({ data }: { data: Awaited<ReturnType<typeof api.overview>> }) {
  ...
  return <div className="overview-grid">
    ...
    <article className="system-card"><div><span className="section-kicker">SYSTEM IDENTITY</span><h2>{data.hostname}</h2><p>{data.osName} {data.osVersion} · {data.currentUser}</p><div className="network-rate"><span>RX {formatBytes(data.networkRxBytesPerSecond)}/s</span><span>TX {formatBytes(data.networkTxBytesPerSecond)}/s</span></div></div><dl><div><dt>内核 / 架构</dt><dd>{data.kernel} · {data.architecture}</dd></div><div><dt>网络</dt><dd>{data.primaryIp || "—"} · gw {data.defaultGateway || "—"}</dd></div><div><dt>平台</dt><dd>{data.packageManager} · systemd {data.systemdRunning ? "running" : "degraded"}</dd></div><div><dt>远端时间</dt><dd>{data.currentTime || "—"} {data.timezone}</dd></div></dl></article>
    <article className="runtime-card"><div className="runtime-card__header"><span className="section-kicker">RUNTIMES</span><Box size={18} /></div><div className="runtime-row"><span className={`runtime-icon ${data.docker.running ? "ok" : ""}`}>D</span><div><strong>Docker</strong><small>{data.docker.installed ? data.docker.version ?? "已安装" : "未安装"}</small></div><em>{data.docker.running ? "运行中" : data.docker.installed ? "已停止" : "不可用"}</em></div><div className="runtime-row"><span className={`runtime-icon ${data.nginx.running ? "ok" : ""}`}>N</span><div><strong>Nginx</strong><small>{data.nginx.installed ? data.nginx.version ?? "已安装" : "未安装"}</small></div><em>{data.nginx.running ? "运行中" : data.nginx.installed ? "已停止" : "不可用"}</em></div><div className="runtime-summary"><span>{data.listeningPorts} listening</span><span className={data.failedServices ? "warn" : ""}>{data.failedServices} failed units</span></div></article>
  </div>;
}
