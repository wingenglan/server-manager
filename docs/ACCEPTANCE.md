...
- [ ] Ubuntu 密码认证真实验证
- [ ] Rocky/Alma 私钥认证真实验证
- [~] 0.3.0 终端快捷指令：默认指令、前缀匹配、Tab 插入、变量填写和全局/服务器覆盖（代码与自动化测试通过，真实 UI/服务器待验收）
...
- 快捷指令：SQLite migration、默认指令、全局/服务器覆盖、标签/启用状态、变量提取、大小写不敏感的前缀/分词匹配、最多 6 条建议、Tab 插入和管理入口已接入；Enter 仍执行终端命令。
