...
密码不会进入 LocalStorage、SQLite、日志或诊断包；Relay 不会把终端历史写入磁盘，但用户在远程 shell 中主动提交的内容会在当前 shell 内存历史中短暂显示，因此不应把密码、私钥或 token 作为命令/输入提交。保存完成后 React 表单清空。Keychain 写入失败时整个保存操作失败，不做明文降级。
...
- 快捷指令、任务记录和审计事件不得持久化密码、私钥、sudo 凭据、完整终端输入或命令输出；界面提示用户使用变量代替敏感值。当前 shell 的短期命令历史也不应输入敏感值。
